...
class ReplayBuffer(object):
    ...
    def sample(self):
        ...
        s_batch = torch.stack([e[0] for e in batch]).squeeze(1)
        a_batch = torch.stack([e[1] for e in batch]).squeeze(1)
        r_batch = torch.stack([e[2] for e in batch]).squeeze(1)
        s2_batch = torch.stack([e[3] for e in batch]).squeeze(1)
        ...
        if len(batch[0]) <= 5:
            ...
            return s_batch, a_batch, r_batch, s2_batch, term_batch, dfdx_batch, dfdu_batch, dcdx_batch, d2cdu2inv_batch
